	...
	var controller = require("../objects/Controller").extend({
		...
		ensureExists:function(req,res,next){
			app.getModel.call(app,getSelf()._model).findById(req.params.id,function(err,record){
				if(err){
					throw new Exception(602, err, res);
				}else if(record){
					req.record = record;
					next();
				}else{
			...
			getSelf().beforeCreate(req, res, function(){
				try{
					...
					app.getModel.call(app,getSelf()._model).create(input,function(err,record){
						if(err){
							throw new Exception(601, err, res);
						}
						else{
		...
		getCollection: function(req, res){
			getSelf().beforeGetCollection(req, res, function(){
				var model = app.getModel.call(app,getSelf()._model);
				var queryConditions = req.queryConditions || {};
				if(req.query.query){
					if(req.query.query.split(":").length > 1)
					{
						...
						for(var i in queryArr){
							...
							if(query.length == 2){
								...
								if(model.schema.paths[query[0]].instance=='ObjectID'){
									queryConditions[query[0]] = query[1];
								}else{
									queryConditions[query[0]] = new RegExp(query[1],"i");
								}
							}
				...
				var pageSize = req.query.per_page?req.query.per_page:app.openbiz.config.client_pageSize;
					...
					if(err){
						...
						getSelf().afterGetCollection(req, res,function(){
							var dataArrayInfo = {
								...
								total_pages: Math.ceil(results.count/(pageSize==0?app.openbiz.config.client_pageSize:pageSize)),
		...
		getCollectionByQuery: function(req, query, callback){
			var pageSize = req.query.per_page?req.query.per_page:app.openbiz.config.client_pageSize;
			var page = req.query.page?req.query.page-1:0;
			...
			async.series({
				...
				if(err){
					...
					callback(dataArrayInfo, req.recordCollection);
				}
			});
		},
			...
			getSelf().beforeDelete(req, res, function(){
				if(typeof req.record._doc.state != "undefined"){
					...
					req.record.remove(function(err){
						if(err){
							throw new Exception(603, err, res);
						}
						else{
							getSelf().afterDelete(req, res);
							res.send(204);
		...
		update:function(req,res){
			getSelf().beforeUpdate(req, res, function(){
				...
				req.record.save(function(err){
					if(err){
						throw new Exception(604, err, res);
					}
					else{
		...
		mapReduce: function(req, mapreduce, callback){
			app.getModel.call(app,getSelf()._model).mapReduce(mapreduce, function (err, results) {

				app.openbiz.db.db.collection(mapreduce.out, function(err, collection) { 
					var pageSize = req.query.per_page?req.query.per_page:app.openbiz.config.client_pageSize;
					var page = req.query.page?req.query.page-1:0;
					...
					async.series({
						count:function(cb){
							...
							query = getSelf().beforeMapReduceQueryCollection(query);
							query.toArray(function(err,collections){
								req.recordCollection = collections;
								cb(err,collections);
							});
						}
					},function(err,results){
						if(err){
		        			...
		        			callback(null, [dataArrayInfo, req.recordCollection]);
						}
					});
			    });  
