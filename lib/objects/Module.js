		...
		fs.readdirSync(modPath).map(function(file){
			if( fs.statSync(path.join(modPath,file)).isFile() 
                ...
                {                	
				    ...
				    var objName = selfPointer._name +"."+path.basename(file,path.extname(file));
				    var obj = selfPointer._app.getController.call(selfPointer._app,objName);
				    selfPointer._controllers[path.basename(file,path.extname(file))] = obj;
                }
		});
	}
});
