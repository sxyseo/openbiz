...
module.exports = function(app,options)
{
    ...
    db.once('open', function () {
        ...
        openbiz.gfs = openbiz.GridFS = gfs;
    });

    //setup for storage user uploaded files
    if(typeof options.storage != 'undefined'){
        if(!fs.existsSync(options.storage.tmpPath)){
                fs.mkdir(options.storage.tmpPath)
            }
        if(options.storage.type.toLowerCase()=='fs'){
            if(!fs.existsSync(options.storage.path)){
                fs.mkdir(options.storage.path)
            }   
            app.use(options.storage.url,express.static(options.storage.path));
        }else if(options.storage.type.toLowerCase()=='gridfs'){
            app.use(options.storage.url, require('./services/GridFS').storageReader);
        }
    }
    ...
    var openbiz =
	{
		...
		{			
            ...
            ExceptionLoader: require("./loaders/ExceptionLoader"),
            NlsLoader:       require("./loaders/NlsLoader"),
            ServiceLoader:   require("./loaders/ServiceLoader")
		},
        /**
		...
		controllers:
		{
            ...
            SingleController: require("./controllers/SingleController")
		},
        /**
		...
		policies:
		{
			ensureDataACLPermission:    require("./policies/ensureDataACLPermission"),
            ensureDataState:            require("./policies/ensureDataState"),
        ...
        services:
        {
            ...
            MetadataParser:    require("./services/MetadataParser"),
            ModelService:      require("./services/ModelService"),
            FileUploader:      require("./services/FileUploader"),
        },
        /**
	...
	//TODO: 注释和测试
	openbiz.ensureDataACLPermission = openbiz.policies.ensureDataACLPermission;

	openbiz.ensureDataState = openbiz.policies.ensureDataState;
	//setup classes alias
    /**
    ...
    openbiz.SingleController = openbiz.controllers.SingleController;
    openbiz.ModelSubdocController = openbiz.controllers.ModelSubdocController;
    openbiz.MetadataParser = openbiz.services.MetadataParser;
    openbiz.ModelService = openbiz.services.ModelService;
    openbiz.FileUploader = openbiz.services.FileUploader;
    
    openbiz.storage = options.storage;

    openbiz.config = options;
