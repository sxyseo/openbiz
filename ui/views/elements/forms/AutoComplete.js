			...
			init:function(metadata,parent,model){
				...
				this._parseModel(function(){
					if(self._modelType == "model"){
						self.collection.fetch({
							success:function(){
								...
								for (var i in self.collection.models){
									data.push({id:i+1,name:self.collection.models[i].get(self._dataSource.displayField)});
								}
								self._parent.$el.find(selector).find("input[name='"+self._metadata.elemName+"']").typeahead({
									source: data
								});
							}
						})
						...
						var data = [];
						for (var i in self._dataSource.model){
							data.push({id:i+1,name:self._dataSource.model[i]});
						}
						self._parent.$el.find(selector).find("input[name='"+self._metadata.elemName+"']").typeahead({
							source:data
						});
